...
<header class="header">
	...
	<h1 class="logo"><a href="<%=basePath%>" title="乱七八糟">乱七八糟</a></h1>
	<ul class="nav">
...
<div class="widget widget_searchbox affix-top" style="top: 0px;">
    <form method="get" class="search-form" action="<%=basePath%>">
        <input class="form-control" id="wxUrl" type="text" placeholder="输入关键字如：暂时不能用">
        <input class="btn" type="button" value="搜一下" onclick="send()">
    </form>
</div>
...
<div class="widget widget_tags">
   ...
   <ul class="widget_tags_inner"> 
   	<c:forEach items="${tagList}" var="tag">
   		<li><a title="" href="javascript:void(0)" onclick="goTag('${tag.tagName}')">${tag.tagName}(${tag.sum})</a></li>
   	</c:forEach>
   </ul> 
  </div> 
...
<script>
    ...
    function send(){
        ...
        if(isEmpty(wxUrl)){
            ...
            $.ajax({
                type : "POST",
                url : getRootPath()+'article/wx',
                data : 'url='+encodeURI(encodeURI(wxUrl)),
