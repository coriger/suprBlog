...
<c:forEach items="${articleList}" var="article">
						...
						<header>
							...
							<h2><a href="<%=basePath%>article/html/${article.id}" title="${article.title}">${article.title}</a></h2>
						</header>
							
						<p class="text-muted time">作者：${article.author}&nbsp;&nbsp;&nbsp;&nbsp; 发布于 ${article.createTime}</p>
						
						<p class="focus">
							<a href="<%=basePath%>article/html/${article.id}" class="thumbnail" style="border:0">
							<span class="item">
								<span class="thumb-span">
									<c:if test="${article.thumbUrl !=null}">
										<img data-original="<%=basePath%>uploads/${article.thumbUrl}" class="thumb" src="<%=basePath%>uploads/loading.jpg" style="display: inline;">
									</c:if>
								</span>
							</span>
							</a>
